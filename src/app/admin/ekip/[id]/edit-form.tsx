...
export function TeamEditForm({ member }: { member: TeamMember }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("Ekip uyesi guncellendi");
      ...
      <CardContent className="p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div><Label htmlFor="title">Unvan *</Label><Input id="title" name="title" defaultValue={member.title} required className="mt-1.5" /></div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><Label htmlFor="specialization">Uzmanlik Alani</Label><Input id="specialization" name="specialization" defaultValue={member.specialization || ""} className="mt-1.5" /></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><Label htmlFor="photoUrl">Fotograf URL</Label><Input id="photoUrl" name="photoUrl" type="url" defaultValue={member.photoUrl || ""} className="mt-1.5" /></div>
            <div><Label htmlFor="sortOrder">Siralama</Label><Input id="sortOrder" name="sortOrder" type="number" defaultValue={member.sortOrder} className="mt-1.5" /></div>
          </div>
